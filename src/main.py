...
def main():
    ...
    for jogo in jogos_do_dia:
        ...
        if odd_casa:
            ...
            if prob_real > prob_implicita:
                ...
                if kelly > 0:
                    apostas_valor.append({"jogo": f"{jogo['time_casa']} vs {jogo['time_fora']}", "odd": odd_casa, "prob": prob_real, "kelly": kelly})
    ...
    if len(apostas_valor) >= 3:
        ...
        for i, aposta in enumerate(bilhete, 1):
            print(f"{i}. {aposta['jogo']} | Odd: {aposta['odd']} | Prob: {aposta['prob']*100:.1f}%")
        print("-" * 60)
        print(f"💵 Odd Final: {odd_final:.2f} | 📌 APOSTAR: R$ {valor_aposta:.2f}")
        print("="*60)
    else:
        print("\n❌ Não há 3 jogos com valor hoje.")
